...
**Files:** `src/app/api/ai/question/route.ts`, `src/app/api/ai/summary/route.ts`, `src/lib/ai/medgemma-client.ts`, `src/lib/ai/mock-medgemma-client.ts`, `src/lib/ai/vertex-medgemma-client.ts`, `src/lib/ai/prompts.ts`, `src/lib/ai/question-schema.ts`, `src/lib/ai/summary-schema.ts`, `src/lib/ai/validators.ts`, `src/lib/safety/safety-rules.ts`, `src/lib/safety/safety-copy.ts`, `src/lib/privacy/build-ai-context.ts`, `tests/unit/ai/validators.test.ts`, `tests/integration/api/medgemma-routes.test.ts`, `tests/e2e/actual-medgemma.spec.ts`
...
**Verification:** mock contract, Route Handler integration, actual Vertex 세 fixture가 각각 통과한다.
...
**Files:** `src/features/speech/use-speech-recognition.ts`, `src/features/speech/voice-input.tsx`, `src/features/speech/use-speech-synthesis.ts`, `src/features/speech/speech-button.tsx`, `src/features/interview/question-card.tsx`, `src/features/interview/safety-notice.tsx`, `src/features/summary/summary-card.tsx`, `src/styles/_speech.scss`, `tests/unit/speech/speech-hooks.test.ts`, `tests/integration/speech/content-controls.test.tsx`, `tests/e2e/speech-controls.spec.ts`

**Approach:** STT는 지원·권한을 검사하고 사용자가 확정한 transcript만 답변 draft에 넣는다. TTS는 화면에 표시된 승인 문장만 읽는다. AI 응답 완료에는 `aria-live`, 오류·안전에는 `role=alert`, 새 질문·요약에는 heading focus를 적용한다.

**Test Scenarios:**

1. 음성 권한 거부·미지원·인식 오류 뒤 text input과 draft가 유지된다.
2. interim transcript는 저장되지 않고 확정한 transcript만 저장된다.
3. user click 전 TTS가 재생되지 않고 새 발화 전 기존 발화를 취소한다.
4. navigation, `visibilitychange`, `pagehide`, reset에서 TTS와 STT가 중단된다.
5. 새 질문, 안전 안내, 요약 전환이 screen reader에 알리고 focus를 예측 가능한 위치로 이동한다.
6. hidden prompt와 direct identifier를 읽지 않는다.

**Verification:** hook·component tests와 Chromium manual STT/TTS smoke가 통과한다.
