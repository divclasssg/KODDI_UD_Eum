...
|---|---|---|---|
| Day 1 | Next.js 설정, SCSS token, 대표 문진 화면, 대상 Chromium과 Modal deployment profile 동결 | lint·typecheck·build; Node Route에서 인증→한국어 질문→JSON 추출·schema 검증→timeout/error mapping | React Compiler off, 사진 제외 |
| Day 2 | IndexedDB, 동의, 기본·의료정보, text·choice·measurement, state machine | onboarding→home, 저장·새로고침 복구 | 부가 의료정보 필드 축소 |
| Day 3 | mock/Modal provider, 실제 질문·요약, safety·schema·evidence validator, manual fallback; fixture로 기록 화면 골격 | 상황 1 text/chip actual MedGemma 완주; summary capability gate | 사진 제외 확정 |
| Day 4 | TTS, 모의 음성 입력, 입력 방식 전환, Persona 규칙; fixture 기반 records·clinician view | 세 Persona Task 1, 모의 transcript fallback, 오늘 기록 선택 규칙 | 음성 시각 효과 축소 |
...
| Day 7 | 상황 1~3 actual rehearsal, TTS·모의 음성·저장·reset 회귀, 버그 수정, README | full tests, actual evidence, production build | 새 기능 금지 |

### Risks and Mitigations

| Risk | Impact | Mitigation |
|---|---|---|
| Modal quota·budget·endpoint 없음 | 실제 MedGemma P0 미완료 | mock/actual 결과 분리, blocker 명시 |
| 실환자용 목표와 공개 데모 보안 불일치 | 민감정보 노출·잘못된 완료 주장 | 합성 Persona 역할극, 직접 식별정보 탐지, quota·kill switch 적용 |
| AI 응답·모의 음성 timer가 reset 뒤 상태를 되살림 | 삭제 실패·기록 오염 | AbortController, timer cleanup, interview revision/request guard |
...
| 모의 음성 transcript 선택 실패 | 입력 경로 중단 | 결정론적 transcript registry와 text fallback |
