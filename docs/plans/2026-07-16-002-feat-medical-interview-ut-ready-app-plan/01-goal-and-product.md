...
7일 안에 문서의 합성 Persona 데이터를 사용하는 의료 문진 프로토타입을 구현한다. 사용자는 실제 MedGemma 문진을 작성하고, 확정한 결과를 의료진용 화면으로 보여 주며, 이전 기록과 내 정보를 관리할 수 있어야 한다. 질문·안전 안내·요약에는 명시적인 TTS를 제공하고, 텍스트·선택형·음성 입력은 실제로 동작해야 한다.

이번 결과물은 AI Persona UT 실행 도구가 아니다. 참가자 AI, 평가자 AI, 스크린샷 중계, 로그 채점, 2/3 재현 판정, 의료진 휴리스틱 배포는 앱 구현 이후의 별도 단계로 미룬다.

이번 7일 결과물은 기능 검증용이며 실제 진료 서비스가 아니다. 인증·저장 암호화·접근통제·감사·보존 정책이 없는 동안 실제 환자, 실제 식별정보, 실제 의료기록을 입력하거나 공개·LAN 배포하지 않는다.
...
- 기간: 7일
- 공식 대상 환경: 데스크톱 Chromium의 393px viewport, `127.0.0.1` loopback
- 핵심 과업: 최초 문진, 의료진에게 결과 보여주기, 이전 기록 확인과 내 정보 수정
- AI: Vertex AI에 직접 배포한 `google/medgemma-1.5-4b-it`
- 저장: 브라우저 IndexedDB, 계정·서버 DB 없음
- 음성 출력: 브라우저 Speech Synthesis, 사용자 명시 실행
- 음성 입력: 대상 Chromium의 Speech Recognition, 브라우저 제공자 처리 가능성 고지·명시 동의·텍스트 fallback; 앱은 오디오를 저장하지 않음
- 디자인: SCSS, Figma 기반 `:root` token, 하이픈 class name
- 데이터 경계: 합성 Persona 데이터만 사용하며 실제 환자·실제 식별정보 입력 금지
- 실행 경계: loopback만 허용하며 실제 휴대폰·LAN·공개 서비스 동작을 완료로 주장하지 않는다.

### Stop Conditions

- Day 1 첫 gate에서 GCP project·billing·GPU quota·Vertex endpoint 실제 응답을 확인한다. 실패하면 mock으로 앱 개발은 계속하되 실제 MedGemma 완료를 주장하지 않는다.
