...
- [Color token reference](./2026-07-16-design-token-foundation-design/01-color-token-reference.md)
- [Measurement, radius, typography reference](./2026-07-16-design-token-foundation-design/02-measurement-typography-reference.md)

두 참조 문서는 승인된 구현값을 정의한다. Figma 원본과 접근성 보정안을 분리해 표시하며, SCSS 구현은 이 명세를 기준으로 한다.
...
- 현재 globals.scss의 다크 모드 media query와 Arial font-family는 승인된 설계와 불일치하므로 구현 단계에서 제거한다.
