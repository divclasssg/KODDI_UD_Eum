...
- 예: 생년월일이 `2010-09-01`이고 오늘이 `2026-07-13`이면 만 나이는 15세다.
...
type MedicalProfile = {
  id: "default";
  heightCm?: number;
  heightConfirmedAt?: string;
  weightKg?: number;
  weightConfirmedAt?: string;
  ...
  smokingConfirmedAt?: string;
  drinking?: "yes" | "no" | "unknown";
  drinkingConfirmedAt?: string;
  createdAt: string;
  updatedAt: string;
};
...
- 각 의료정보에는 마지막으로 사용자가 확인한 시각을 저장한다.
- 사용자가 `모르겠음`을 선택하면 기존 값과 마지막 확인 시각을 덮어쓰지 않는다.
...
- 선택 버튼 클릭 즉시 저장을 시작하고 텍스트 입력창은 변경하지 않는다.
- 저장 중에는 모든 선택 버튼을 비활성화해 중복 답변을 막는다.
- 저장 실패 시 현재 질문과 선택 상태를 유지한다.
