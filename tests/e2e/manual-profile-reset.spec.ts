...
async function expectExactPathname(page: Page, expected: string) {
  await expect.poll(() => new URL(page.url()).pathname).toBe(expected);
}

async function tabTo(page: Page, target: Locator, maximumTabs = 100) {
...
test("프로필 수정은 과거 snapshot을 바꾸지 않고 새 완료 기록에만 반영한다", async ({
  ...
  const firstRecordPath = `/records/${encodeURIComponent(firstRecordId)}`;
  await expectExactPathname(page, firstRecordPath);
...
test("프로필 수정의 저장과 취소 확인을 키보드만으로 완료한다", async ({
  page,
}) => {
  await completeSyntheticOnboarding(page);
  await completeManualInterview(page, "키보드");
  const recordId = await readFirstCompletedRecordId(page);
  const recordPath = `/records/${encodeURIComponent(recordId)}`;
  await page.goto(recordPath);
  await expectExactPathname(page, recordPath);

  const editLink = page.getByRole("link", { name: "내 정보 수정" });
  await tabTo(page, editLink);
  await expect(editLink).toBeFocused();
  await page.keyboard.press("Enter");
  await expectExactPathname(page, "/profile");

  const name = page.getByLabel("이름");
  await tabTo(page, name);
  await expect(name).toBeFocused();
  await page.keyboard.press("ControlOrMeta+A");
  await page.keyboard.type("키보드 저장 사용자");
  ...
  await tabTo(page, name);
  await page.keyboard.press("ControlOrMeta+A");
  await page.keyboard.type("키보드 취소 사용자");

  const cancel = page.getByRole("button", { name: "취소하고 돌아가기" });
  await tabTo(page, cancel);
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("heading", { name: "변경사항을 버릴까요?" }),
  ).toBeVisible();

  const continueEditing = page.getByRole("button", { name: "계속 수정" });
  await tabTo(page, continueEditing);
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("heading", { name: "변경사항을 버릴까요?" }),
  ).not.toBeVisible();
  await expect(name).toHaveValue("키보드 취소 사용자");

  await tabTo(page, cancel);
  await page.keyboard.press("Enter");
  const discard = page.getByRole("button", { name: "변경사항 버리기" });
